...
function App() {
  ...
  const loadResponse = async (cycleId: string, day: number) => {
    const { data } = await supabase
      .from('responses')
      .select('evening_response')
      .eq('cycle_id', cycleId)
      .eq('day', day)
      .maybeSingle();

    setResponse(data?.evening_response || '');
  };
    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <p className="text-xs text-gray-400 mb-1">Nap {cycle.current_day}/30</p>
          <h2 className="text-sm font-light text-gray-800">{dayData.morning}</h2>
        </div>

        <div className="space-y-4">
          <div>
            <label className="text-xs text-gray-600 block mb-2">{dayData.evening}</label>
            <input
              type="text"
              value={response}
